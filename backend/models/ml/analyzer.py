...
class Analyzer():
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not os.path.exists("figures"):
            os.makedirs("figures")
        root_path = "/mnt/c/GIT/SIZ/Savy-Savvy/frontend/static/figures/"
    ...
    def get_price_over_time_fig(self, item="all", x="updated_at", **kwargs):
        ...
        path = f"{root}/figures/price_over_time_fig_{itm}-{x}.png"
        plt.savefig(path)
    ...
    def get_price_range(self, item="all", **kwargs):
        ...
        itm = str(list(df['type'])[0]).replace(" ", "_")
        root = "/mnt/c/GIT/SIZ/Savy-Savvy/frontend/static"
        path = f"{root}/figures/violin_price_range_{itm}.png"
        plt.savefig(path)
